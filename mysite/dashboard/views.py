...
from .models import Pop, Hospital, Medicine, Population, Medicalinfo, Counts
from django.db.models import Sum

def dashboard_view(request):

    pop_data = Pop.objects.all().order_by('type')
    pop_types = [data.type for data in pop_data]
    pop_ms = [data.m for data in pop_data]
    pop_fs = [data.f for data in pop_data]
